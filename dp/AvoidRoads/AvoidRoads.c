...
unsigned long long int d[MAX][MAX];
const int width=19;
...
const int n=sizeof(bad)/4; // elements count in bad
...
int main()
{
	...
	printf("%d\n",n);
	int i,j;
	for(i=0;i<n;i++){// input the bad block list
		int a,b,c,d;
	...
	unsigned long long int a=d[height][width];
	printf("%lld\n",a);
	/*
	for(j=0;j<=height;j++){
		for(i=0;i<=height;i++){
