...
// The string defined by this object is:
// Start from origin_node, trace a path to this node,
// concatenate all the chars encountered in the path, 
// finally append the substring [first_char_index, last_char_index]
class Suffix {
    public :
        int origin_node;
...
//   A----B original
// =>   A--C--B split
// where A--C is the new edge, and C--B is the original edge where start_node
// is changed from A to C

int Edge::SplitEdge( Suffix &s )
{
    Remove();
    Edge *new_edge =
        // the original string in this edge is splited to two strings
      new Edge( first_char_index, // from the first char
               // the shift is length of the suffix in active node - 1
               // the following is actually
               // first_char_index + len - 1
               // where len = s.last_char_index - s.first_char_index + 1
                first_char_index + s.last_char_index - s.first_char_index,
                s.origin_node );
    // The splitting means the suffix is a prefix of some suffix
    new_edge->Insert();
    Nodes[ new_edge->end_node ].suffix_node = s.origin_node;
    first_char_index += s.last_char_index - s.first_char_index + 1;
    start_node = new_edge->end_node;
    Insert();
    // important: return the new node index
...
void AddPrefix( Suffix &active, int last_char_index )
{
    ...
    // This for loop process the suffix from active point to end point
    for ( ; ; ) {
        Edge edge;
...
// out of this big loop, i.e., we reach at the END POINT
//
        if ( active.Explicit() ) {
            edge = Edge::Find( active.origin_node, T[ last_char_index ] );
            if ( edge.start_node != -1 ) {
                //cout << "Find explicit node and edge" << endl;
                break;
            }
            //cout << "Find explicit node, but no edge" << endl;
            ...
            if ( T[ edge.first_char_index + span + 1 ] == T[ last_char_index ] ) {
                //cout << "Find implicit node and edge" << endl;
                break;
            }

            //cout << "Split edge" << endl;
            // note that after spliting, the origin_node of the suffix s
            // should be the newly created node
...
//
// Note that his newly created edge will connect to a new node,
// which is a *leaf node*! The index of this leaf node is assigned as 
// (number of present nodes + 1)
        Edge *new_edge = new Edge( last_char_index, N, parent_node );
        new_edge->Insert();
        if ( last_parent_node > 0 ) // don't do this for root node
            Nodes[ last_parent_node ].suffix_node = parent_node;
        // now we processed this node, it becomes last parent node
...
//
        if ( active.origin_node == 0 )
          ...
          // Note: the precondition here is that
          // the active node is canonized, hence, if it is explicit,
          // the origin_node is the explicit node itself
          // if it is implicit, then it is the nearest parent node
          // Also, it CANNOT be a leaf node, otherwise it is not active node.
          // (Active node is a non-leaf node, either implicit or explicit)
...
int main()
{
    ...
    N = strlen( T ) - 1;  // N is the last index of the string, not length
//
